...
class Lattice(object):
    ...
    def evolve(self,n_steps):
        for t in range(n_steps):
            ...
            neighb3x=np.ravel(self.lattice[i-1:i+2,j-1:j+2])
  
            altruist_count=len(np.where(neighb5x=='b')[0])
            growth_bonus=(altruist_count**10)/(12**10)
            
            #respond to neighborhood
            

            
            if self.lattice[i,j]!='0': #site is NOT empty
                n0=len(np.where(neighb3x=='0')[0])
                if n0>=0:
                    zeros=zip(np.where(self.lattice[i-1:i+2,j-1:j+2]=='0')[0],np.where(self.lattice[i-1:i+2,j-1:j+2]=='0')[1])
                    if growth_bonus*np.random.random()>.5:
                        if self.lattice[i,j]=='r':
                            for site in zeros:
                                self.lattice[site[0],site[1]]=self.lattice[i,j]
                        elif self.lattice[i,j]=='b':
                           for site in zeros:
                               if np.random.random()>.25:
                                   self.lattice[site[0],site[1]]='b'
    
                if np.random.random()>.99:
                    self.lattice[i,j]='0'
                    if 'r' or 'b' in neighb3x:
                        if np.random.random()*growth_bonus>.5:
                            nb=len(np.where(neighb3x=='b')[0])
                            nr=len(np.where(neighb3x=='r')[0])
                            if nr-nb>=-1 and nr>=1:
                                self.lattice[i,j]='r'
                            elif nb>0:
                                self.lattice[i,j]='b'
                                ...
                                print("error in filling")
                    #enemyColor=self.colors[self.colors!=self.lattice[i,j]][0]
                    #enemyCount=len(np.where(neighb==enemyColor))
                    #if self.lattice[i,j]=='b':
                    #    enemyCount*=5
                    #if (enemyCount/4)>np.random.random():
                    #    self.lattice[i,j]=='0'
                
                    #self.lattice[i,j]=self.colors[np.argmax([np.random.random()*len([i for i in neighb if i=='r']),np.random.random()*len([i for i in neighb if i=='b'])])]
...
def video(lattice):
    for t in range(50):
        im=lattice.view()
        im.save('coopVid%d.png'%t)
        lattice.evolve(400000)
    return
